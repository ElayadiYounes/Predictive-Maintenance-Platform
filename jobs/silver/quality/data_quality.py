...
class InspectionDataQuality :
    ...
    LIMITE_EXPECTED_SCHEMA ={
        "instal" : StringType,
        "t_av_limite" : DoubleType,
        "v_ax_limite" : DoubleType,
        "v_h_limite" : DoubleType,
        "v_v_limite" : DoubleType
    }
    MAX_NULL_RATIO = 0.3
    ...
    def limite_validate_required_columns(self, dataframe: DataFrame, ) -> None:
        """
        Vérifie que toutes les colonnes métier
        obligatoires sont présentes.
        ...
        missing_columns = sorted(
            set(self.LIMITE_REQUIRED_COLUMNS)
            - set(dataframe.columns)
        )
    ...
    def limite_validate_schema(self, dataframe: DataFrame) -> None:
        """
        Vérifie que le schéma Spark correspond
        au schéma attendu.
        ...
        for column, expected_type in self.LIMITE_EXPECTED_SCHEMA.items():
    ...
    def limite_validate_null_ratio(self, dataframe: DataFrame) -> None:
        """
        Vérifie que le pourcentage de valeurs
        manquantes de chaque colonne reste
        inférieur au seuil autorisé.
